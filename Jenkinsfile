    ...
    stage('Deploy Dev') {
      ...
      steps {
        ...
        withCredentials([file(credentialsId: 'kubeconfig-aiml-lower-env', variable: 'KUBECONFIG')]) {
          ...
          sh("kubectl --cluster=exp get ns ${env.BRANCH_NAME} || kubectl --cluster=exp create ns ${env.BRANCH_NAME}")
          // Don't use public load balancing for development branches
          sh("sed -i.bak 's#LoadBalancer#ClusterIP#' ./k8s/services/frontend.yaml")
          sh("sed -i.bak 's#gcr.io/cloud-solutions-images/gceme:1.0.0#${imageTag}#' ./k8s/dev/*.yaml")
          sh("kubectl --namespace=${env.BRANCH_NAME} --cluster=cluster apply -f k8s/services/")
          sh("kubectl --namespace=${env.BRANCH_NAME} --cluster=cluster apply -f k8s/dev/")
